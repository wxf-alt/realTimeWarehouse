...
import bean.{AlertInfo, EventLog, OrderDetail, OrderInfo, SaleDetail}
...
object SaleApp {
    ...
    orderInfoStream.fullOuterJoin(orderDetailStream)
      .mapPartitions(it => {
        ...
        val result: Iterator[SaleDetail] = it.flatMap {
          ...
          // 2.orderIdfo没有对应的数据，但是orderDetail存在
  ...
  def main(args: Array[String]): Unit = {
    ...
    val saleDetailStream: DStream[SaleDetail] = funllJoin(orderInfoStream, orderDetailStream)
